...
_a = Middleware;
Middleware.Authenticate = (role) => (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (typeof result === 'string') {
            ...
            const decode = yield Auth_1.AUTH.findOne({ active: true, _id: result._id });
            const currentDate = new Date().getTime();
            console.log(new Date(currentDate).getHours(), new Date(result.exp).getHours());
            ...
            if (!isAllowed) {
                helper_1.logger.error('unauthorized access');
